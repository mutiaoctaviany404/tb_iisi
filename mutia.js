function caripaketmutia()
{
  $('#hasilcaritempat').empty();
  hapusInfo();
  hapusmarker();
  
  var txt = document.getElementById('souvenirmutia').value;
  var sel = document.getElementById('kecamatanmutia').value;
  if(txt=='' || sel == '')
  {
  ...
  {
    $.ajax({ 
        url: server+'mutia.php?kecamatan='+sel+'&souvenir='+txt, data: "", dataType: 'json', success: function(rows)
          { 
            if(rows==null)
            {
