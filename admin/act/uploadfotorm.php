<?php 
	...
	$id = $_POST['id'];
	$querysearch="select no_urut from restaurant_gallery where id_restaurant='$id' order by no_urut desc limit 1";
	...
	if(($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/gif"  || $jenis_gambar=="image/png") && ($_FILES["image"]["size"] <= 5000000)){
		...
		$sql = pg_query("insert into restaurant_gallery values('$no_urut','$name','$id')");
		if($sql){
