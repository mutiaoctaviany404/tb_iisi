<?php 
	...
	$id = $_POST['id'];
	$querysearch="select no_urut from souvenir_gallery where id_souvenir='$id' order by no_urut desc limit 1";
	...
	if(($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/gif"  || $jenis_gambar=="image/png") && ($_FILES["image"]["size"] <= 5000000)){
		...
		$sql = pg_query("insert into souvenir_gallery values('$no_urut','$name','$id')");
		if($sql){
		...
		echo "Go Back To <a href='../index1.php?page=detailsouvenir&id=$id'></a>";
	}
?>