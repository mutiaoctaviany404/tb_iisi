function caripaketyori()
{
  $('#hasilcaritempat').empty();
  hapusInfo();
  hapusmarker();
  
  var txt = document.getElementById('reastaurantyori').value;
  var sel = document.getElementById('kecamatanyori').value;
  if(txt=='' || sel == '')
  {
  ...
  {
	console.log(server+'yori.php?kecamatan='+sel+'&restaurant='+txt);
    $.ajax({ 
        url: server+'yori.php?kecamatan='+sel+'&restaurant='+txt, data: "", dataType: 'json', success: function(rows)
          { 
            if(rows==null)
            {
